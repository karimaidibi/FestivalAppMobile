...
class FestivalService {
    ...
    func addFestival(festivalDTO : FestivalDTO) async -> Result<FestivalDTO?, Error> {
        ...
        guard let encoded : Data = await JSONHelper.encode(data: festivalDTO) else {
        ...
        do {
            ...
            if httpresponse.statusCode == 201{
                // recuperer query result
                guard let decoded : QueryResultGetOne<FestivalDTO> = await JSONHelper.decodeOne(data: data) else{
                    return .failure(JSONError.JsonDecodingFailed)
                }
                let festivalDTO = decoded.result
                return .success(festivalDTO)
            }
    ...
    func removeFestival(id: String) async -> Result<String?, Error> {
        // Define URL
        ...
        do {
            ...
            if let httpResponse = response as? HTTPURLResponse {
                ...
                // If everything is OK
                if statusCode == 204 {
                    return .success("Festival deleted successfully.")
                } else {
                ...
                return .failure(APIRequestError.invalidHTTPResponse("removeFestival"))
            }
        }
        // Handle any error
        catch {
