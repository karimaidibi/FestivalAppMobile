    ...
    var body: some View {
        NavigationView {
            List(festivals.sorted { $0.isActive && !$1.isActive }) { festival in
                NavigationLink(destination: FakeFestivalView(viewModel: festivalsVM.getFestivalViewModel(for: festival)!)) {
                    FakeFestivalRow(viewModel: festivalsVM.getFestivalViewModel(for: festival)!)
                }
            }
